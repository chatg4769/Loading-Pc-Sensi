    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 429 || response.status >= 500) {
          ...
          console.error(
            "API request failed with non-retriable error:",
            errorResult
          );
          throw new Error(`API error: ${response.statusText}`);
        }
...
const SpeechButton = ({ textToSpeak }) => {
        ...
        contents: [
          {
            parts: [
              {
                text: `Say in a clear, encouraging voice for a gamer: ${textToSpeak}`,
              },
            ],
          },
        ],
  ...
  return (
    ...
    >
      {status === "loading" && (
        <Loader size={20} className="text-cyan-300 animate-spin" />
      )}
      ...
      <div className="relative bg-gradient-to-br from-gray-900/95 to-gray-800/95 backdrop-blur-xl border border-cyan-500/30 rounded-3xl shadow-2xl p-8 text-center max-w-md w-full transform hover:scale-105 transition-all duration-500 overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-gray-300 mb-8 leading-relaxed">
            Access premium sensitivity configurations by subscribing to our
            YouTube channel. Your support fuels our mission to create better
            gaming content!
          </p>
...
const OwnerPanel = ({
  ...
  const handleNewPresetChange = (field, value) => {
    if (field === "ram" || field === "dpi") {
      ...
      setNewPreset((prev) => ({
        ...prev,
        settings: { ...prev.settings, [field]: parseInt(value) || 0 },
      }));
    }
  };
  ...
  const handleDeleteCharacter = (charToDelete) => {
    const updatedCharacters = characters.filter(
      (char) => char !== charToDelete
    );
    setCharacters(updatedCharacters);
  };

  const sortedPresetEntries = Object.entries(presets).sort(
    ([ramA], [ramB]) => parseInt(ramA) - parseInt(ramB)
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fadeIn">
      <div className="relative bg-gradient-to-br from-gray-900/95 to-gray-800/95 backdrop-blur-xl border border-purple-500/30 rounded-3xl shadow-2xl p-8 max-w-4xl w-full h-[90vh] overflow-hidden flex flex-col">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-400 hover:text-white text-2xl w-8 h-8 rounded-full hover:bg-white/10 flex items-center justify-center transition-all z-20"
        >
          ×
        </button>
        <h2 className="text-3xl font-bold bg-gradient-to-r from-purple-400 to-indigo-400 bg-clip-text text-transparent mb-6 text-center font-orbitron">
          Owner's Dashboard
        </h2>
              ...
              {sortedPresetEntries.map(([ram, data]) => (
                ...
                >
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button
                      onClick={() => handleDeletePreset(ram)}
                      disabled={isSaving}
                      className="p-2 text-red-500 hover:bg-red-500/20 rounded-full transition-colors disabled:opacity-50"
                    >
                      <Trash2 size={20} />
                    </button>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        value={data.dpi}
                        onChange={(e) =>
                          handlePresetChange(ram, "dpi", e.target.value)
                        }
                        className="w-full bg-gray-900/70 border border-gray-600 rounded-lg p-2 focus:ring-cyan-400 focus:border-cyan-400"
                      />
                    </div>
                    {Object.entries(data.settings).map(([key, value]) => (
                      <div key={key}>
                        ...
                        <input
                          ...
                          value={value}
                          onChange={(e) =>
                            handlePresetChange(ram, key, e.target.value)
                          }
                          className="w-full bg-gray-900/70 border border-gray-600 rounded-lg p-2 focus:ring-cyan-400 focus:border-cyan-400"
                        />
                      </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-end">
                  <div>
                    ...
                    <input
                      ...
                      value={newPreset.ram}
                      onChange={(e) =>
                        handleNewPresetChange("ram", e.target.value)
                      }
                      className="w-full bg-gray-900/70 border border-gray-600 rounded-lg p-2"
                    />
                  </div>
                  <div>
                    <label className="block text-sm text-gray-400 mb-1">
                    ...
                    <input
                      ...
                      value={newPreset.dpi}
                      onChange={(e) =>
                        handleNewPresetChange("dpi", e.target.value)
                      }
                      className="w-full bg-gray-900/70 border border-gray-600 rounded-lg p-2"
                    />
                  </div>
                  {Object.keys(newPreset.settings).map((key) => (
                    <div key={key}>
                      ...
                      <input
                        type="number"
                        value={newPreset.settings[key]}
                        onChange={(e) =>
                          handleNewPresetChange(key, e.target.value)
                        }
                        className="w-full bg-gray-900/70 border border-gray-600 rounded-lg p-2"
                      />
                    </div>
                  ...
                  <button
                    ...
                    className="col-span-2 md:col-span-1 h-10 mt-2 bg-green-500 hover:bg-green-400 text-white font-bold rounded-lg flex items-center justify-center gap-2 transition-colors"
                  >
              ...
              <div className="p-6 bg-gray-800/50 rounded-2xl border border-gray-700 mb-6">
                ...
                <div className="flex gap-4">
                  <input
                    ...
                    className="flex-1 bg-gray-900/70 border border-gray-600 rounded-lg p-3 focus:ring-cyan-400 focus:border-cyan-400"
                  />
                  <button
                    onClick={addNewCharacter}
                    className="bg-cyan-500 hover:bg-cyan-400 text-white font-bold py-3 px-6 rounded-lg flex items-center justify-center gap-2 transition-colors"
                  >
                    <Plus size={20} />
              ...
              <div className="p-6 bg-gray-800/50 rounded-2xl border border-gray-700">
                ...
                <ul className="space-y-2 max-h-96 overflow-y-auto">
                  {characters.map((char, index) => (
                    <li
                      key={index}
                      className="flex justify-between items-center bg-gray-900/50 p-3 rounded-lg text-gray-300"
                    >
                      <span>{char}</span>
                      <button
        ...
        <div className="pt-6 mt-auto border-t border-gray-700 text-center">
          <button
            ...
            disabled={isSaving}
            className="bg-cyan-500 hover:bg-cyan-400 text-white font-bold py-3 px-8 rounded-lg flex items-center justify-center gap-2 transition-colors w-full md:w-auto mx-auto disabled:opacity-50"
          >
            {isSaving ? (
...
const AIAssistantPanel = ({
  ...
  useEffect(() => {
    if (
      ...
      !popularCharacters.includes(character)
    ) {
  ...
  const handleFeatureGeneration = async (feature) => {
    ...
    try {
      ...
      if (!textResponse)
        throw new Error(`No content received from API for ${feature}.`);
      ...
      if (!parsedData[jsonKeyCheck])
        throw new Error(`Invalid data structure for ${feature}.`);

      switch (feature) {
        case "loadout":
          setWeaponLoadout(parsedData);
          break;
        case "training":
          setTrainingPlan(parsedData);
          break;
        case "optimization":
          setOptimizationTips(parsedData);
          break;
        case "strategy":
          setStrategy(parsedData);
          break;
        case "synergy":
          setSynergy(parsedData);
          break;
      }
    } catch (e) {
    ...
    >
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <select
              ...
              className="w-full bg-gray-900/50 border-2 border-gray-600/50 rounded-lg py-2 px-4 focus:outline-none focus:ring-2 focus:ring-cyan-400 transition-all"
            >
            ...
            <button
              onClick={() => handleFeatureGeneration("loadout")}
              disabled={loadingFeature}
              className="w-full group relative bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold py-3 px-6 rounded-lg flex items-center justify-center gap-2 transition-all transform hover:scale-105 disabled:opacity-50"
            >
              {loadingFeature === "loadout" ? (
            ...
            {weaponLoadout && (
              <div className="p-4 bg-black/20 rounded-lg animate-fadeIn">
                <div className="flex justify-between items-start">
                  <h4 className="font-bold text-cyan-300 text-xl mb-2">
                    {weaponLoadout.loadoutName}
                  </h4>
                  <SpeechButton
                    textToSpeak={`Loadout: ${weaponLoadout.loadoutName}. Primary: ${weaponLoadout.primaryWeapon}. Secondary: ${weaponLoadout.secondaryWeapon}. Rationale: ${weaponLoadout.rationale}`}
                  />
                </div>
                <p>
                  <strong>Primary:</strong> {weaponLoadout.primaryWeapon}
                </p>
                <p>
                  <strong>Secondary:</strong> {weaponLoadout.secondaryWeapon}
                </p>
          ...
          <div className="space-y-4">
            ...
            <button
              onClick={() => handleFeatureGeneration("synergy")}
              disabled={loadingFeature}
              className="w-full group relative bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold py-3 px-6 rounded-lg flex items-center justify-center gap-2 transition-all transform hover:scale-105 disabled:opacity-50"
            >
              {loadingFeature === "synergy" ? (
                <Loader className="animate-spin" size={20} />
              ) : (
                <Rabbit size={20} />
              )}
            ...
            {synergy && (
              <div className="p-4 bg-black/20 rounded-lg animate-fadeIn">
                <div className="flex justify-between items-start">
                  <h4 className="font-bold text-cyan-300 text-xl mb-2">
                    {synergy.combinationName}
                  </h4>
                  <SpeechButton
                    textToSpeak={`Synergy: ${
                      synergy.combinationName
                    }. Skills: ${synergy.passiveSkills.join(", ")}. Pet: ${
                      synergy.pet
                    }. Rationale: ${synergy.synergyRationale}`}
                  />
                </div>
                <p>
                  <strong>Passive Skills:</strong>{" "}
                  {synergy.passiveSkills.join(", ")}
                </p>
                <p>
                  <strong>Pet:</strong> {synergy.pet}
                </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <button
            onClick={() => handleFeatureGeneration("training")}
            disabled={loadingFeature}
            className="w-full group relative bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold py-3 px-6 rounded-lg flex items-center justify-center gap-2 transition-all transform hover:scale-105 disabled:opacity-50"
          >
            {loadingFeature === "training" ? (
              <Loader className="animate-spin" size={20} />
            ) : (
              <Dumbbell size={20} />
            )}
          ...
          <button
            onClick={() => handleFeatureGeneration("optimization")}
            disabled={loadingFeature}
            className="w-full group relative bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold py-3 px-6 rounded-lg flex items-center justify-center gap-2 transition-all transform hover:scale-105 disabled:opacity-50"
          >
            {loadingFeature === "optimization" ? (
              <Loader className="animate-spin" size={20} />
            ) : (
              <Zap size={20} />
            )}
          ...
          <button
            onClick={() => handleFeatureGeneration("strategy")}
            disabled={loadingFeature}
            className="w-full group relative bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold py-3 px-6 rounded-lg flex items-center justify-center gap-2 transition-all transform hover:scale-105 disabled:opacity-50"
          >
            {loadingFeature === "strategy" ? (
              <Loader className="animate-spin" size={20} />
            ) : (
              <Swords size={20} />
            )}
        ...
        {trainingPlan && (
          <div className="p-4 bg-black/20 rounded-lg animate-fadeIn">
            <div className="flex justify-between items-start">
              <h4 className="font-bold text-cyan-300 text-xl mb-2">
                Your 3-Day Aim Training Plan
              </h4>
              <SpeechButton
                textToSpeak={`Your 3-Day Aim Training Plan. Day 1: ${trainingPlan.day1}. Day 2: ${trainingPlan.day2}. Day 3: ${trainingPlan.day3}.`}
              />
            </div>
            <p>
              <strong>Day 1:</strong> {trainingPlan.day1}
            </p>
            <p>
              <strong>Day 2:</strong> {trainingPlan.day2}
            </p>
            <p>
              <strong>Day 3:</strong> {trainingPlan.day3}
            </p>
          </div>
        )}
        {optimizationTips && (
          <div className="p-4 bg-black/20 rounded-lg animate-fadeIn">
            <div className="flex justify-between items-start">
              ...
              <SpeechButton
                textToSpeak={`Device Optimization Tips. Performance: ${optimizationTips.performanceTips}. Display: ${optimizationTips.displayTips}. Network: ${optimizationTips.networkTips}.`}
              />
            </div>
            <p>
              <strong>Performance:</strong> {optimizationTips.performanceTips}
            </p>
            <p>
              <strong>Display:</strong> {optimizationTips.displayTips}
            </p>
            <p>
              <strong>Network:</strong> {optimizationTips.networkTips}
            </p>
          </div>
        )}
        {strategy && (
          <div className="p-4 bg-black/20 rounded-lg animate-fadeIn">
            <div className="flex justify-between items-start">
              ...
              <SpeechButton
                textToSpeak={`AI Game Strategy. Early Game: ${strategy.earlyGame}. Mid Game: ${strategy.midGame}. Late Game: ${strategy.lateGame}.`}
              />
            </div>
            <p>
              <strong>Early Game:</strong> {strategy.earlyGame}
            </p>
            <p>
              <strong>Mid Game:</strong> {strategy.midGame}
            </p>
            <p>
              <strong>Late Game:</strong> {strategy.lateGame}
            </p>
          </div>
        )}
      </div>
...
const AIChatBox = ({ settings }) => {
  const [messages, setMessages] = useState([
    {
      from: "ai",
      text: "Ask me about Free Fire, sensitivity, or mobile optimization!",
    },
  ]);
      ...
      <div className="flex flex-col h-96">
        <div className="flex-1 space-y-4 p-4 overflow-y-auto bg-black/20 rounded-lg">
          ...
          {isTyping && (
            <div className="flex justify-start">
              <div className="px-4 py-3 rounded-2xl bg-gray-700 text-gray-200 rounded-bl-none animate-pulse">
                AI is typing...
              </div>
            </div>
          )}
        ...
        <div className="mt-4 flex gap-4">
          ...
          <button
            ...
            disabled={isTyping}
            className="bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold p-4 rounded-lg flex items-center justify-center transition-all transform hover:scale-105 disabled:opacity-50"
          >
            <Send size={24} />
          </button>
        </div>
...
function App() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (presetsSnap.exists()) {
          ...
          console.warn(
            "No presets document in Firestore, using local fallback data."
          );
  ...
  const checkDeviceAndGetSpecs = async () => {
    ...
    try {
      ...
      if (parsedData.isModelValid && parsedData.ramOptions.length > 0) {
        ...
        setError(
          `No mobile name found for "${trimmedModel}". Please check the spelling and try again.`
        );
      }
    } catch (e) {
  ...
  const sensitivityIcons = {
    ...
    redDot: (
      <div className="w-8 h-8 rounded-full bg-red-500 border-2 border-white shadow-lg"></div>
    ),
    twoXScope: (
    ...
    sniperScope: (
      <div className="text-lg font-bold font-orbitron bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">
        AWM
      </div>
    ),
  ...
  return (
    <div
      style={mainStyle}
      className="min-h-screen text-white font-roboto flex flex-col items-center relative overflow-x-hidden"
    >
      <div className="fixed top-0 left-0 w-full h-full -z-20 overflow-hidden bg-[#0a0d14]">
        <video
          className="absolute top-0 left-0 w-full h-full object-cover opacity-50 transition-opacity duration-1000 ease-in-out"
          src={background}
          loop
          muted
          autoPlay
        ></video>
        <div className="absolute top-0 left-0 w-full h-full bg-radial-to-b from-[rgba(0, 69, 231, 0.9)] to-[rgba(13, 86, 255, 0.95)] -z-10"></div>
      </div>
      ...
      <div className="relative z-10 w-full p-4 sm:p-6 md:p-8">
        {currentStep === "results" && !isUnlocked && (
          <SubscriptionModal
            onUnlock={() => setIsUnlocked(true)}
            channelUrl={YOUTUBE_CHANNEL_LINK}
          />
        )}
        {showAdminLogin && (
          <AdminLoginModal
        ...
        {showOwnerPanel && (
          <OwnerPanel
            ...
            initialPresets={sensitivityPresets}
            initialCharacters={popularCharacters}
        ...
        <div className="w-full max-w-6xl mx-auto">
          <header className="text-center mb-12 animate-fadeInDown">
              ...
              <div className="relative bg-gray-900 rounded-3xl p-2 h-full w-full flex items-center justify-center">
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.src =
                      "https://placehold.co/150x150/000000/00f5ff?text=Loading+PC";
                  }}
                />
              </div>
            </div>
            ...
            <p className="text-gray-300 text-xl mb-6 max-w-2xl mx-auto leading-relaxed">
              Revolutionary AI-powered Free Fire sensitivity optimizer for peak
              performance
            </p>
            ...
            <div className="bg-gradient-to-br from-gray-800/60 to-gray-700/60 backdrop-blur-lg p-8 rounded-3xl shadow-2xl mb-12 border border-gray-600/30 animate-fadeInUp">
              <div className="flex flex-col lg:flex-row items-center gap-6">
                <div className="relative w-full lg:flex-1">
                  <Smartphone
                    className="absolute left-6 top-1/2 -translate-y-1/2 text-cyan-400 z-10"
                    size={24}
                  />
                  <input
                    type="text"
                    ...
                    onKeyPress={(e) =>
                      e.key === "Enter" && checkDeviceAndGetSpecs()
                    }
                    autoComplete="off"
                  />
          ...
          {currentStep === "ram_selection" && deviceSpecs && (
            <div className="bg-gradient-to-br from-gray-800/60 to-gray-700/60 backdrop-blur-lg p-8 rounded-3xl shadow-2xl mb-12 border border-gray-600/30 animate-fadeInUp">
              <h2 className="text-3xl font-bold text-center mb-2 font-orbitron bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">
                {deviceSpecs.modelName}
              </h2>
              <p className="text-center text-gray-400 mb-8 text-lg">
                {deviceSpecs.specsSummary}
              </p>
                  ...
                  {ramOptions.map((ram) => (
                    <button
                      ...
                      className={`px-8 py-4 rounded-xl font-bold text-lg transition-all duration-300 transform hover:scale-105 border-2 ${
                        selectedRam === ram
                          ? "bg-cyan-500/20 border-cyan-400 text-white shadow-cyan-500/20 shadow-lg"
                          : "bg-gray-700/50 border-gray-600 text-gray-300 hover:bg-gray-700/80 hover:border-gray-500"
                      }`}
                    >
                      {ram}
                    </button>
            ...
            <div className="animate-fadeIn space-y-10">
              ...
              <div>
                <h2 className="text-4xl font-bold mb-8 text-center flex items-center justify-center gap-4 bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent font-orbitron">
                  ...
                  <span>
                    Optimized Sensitivity ({mobileModel} - {selectedRam})
                  </span>
                </h2>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {Object.entries(generatedSettings.fixedSensitivity).map(
                    ([key, value]) => (
                      <SensitivityItem
                        key={key}
                        label={key}
                        value={value}
                        icon={sensitivityIcons[key]}
                      />
                    )
                  )}
                </div>
              ...
              <InfoCard
                title="How to Change Your DPI"
                icon={<Info size={28} />}
                gradient="from-gray-700/20 to-gray-600/20"
              >
                <ol className="list-decimal list-inside space-y-4 text-lg">
                  <li>
                    Go to <strong className="text-cyan-400">Settings</strong> on
                    your phone, then find and tap on{" "}
                    <strong className="text-cyan-400">"About Phone"</strong>.
                  </li>
                  <li>
                    Find the{" "}
                    <strong className="text-cyan-400">"Build Number"</strong>{" "}
                    and tap on it 7 times in a row. You'll see a message saying
                    "You are now a developer!"
                  </li>
                  <li>
                    Go back to the main Settings menu and find the new{" "}
                    <strong className="text-cyan-400">
                      "Developer Options"
                    </strong>{" "}
                    menu (it might be under "System" or "Additional Settings").
                  </li>
                  <li>
                    Inside Developer Options, scroll down to the "Drawing"
                    section and find{" "}
                    <strong className="text-cyan-400">"Smallest width"</strong>{" "}
                    or{" "}
                    <strong className="text-cyan-400">"Minimum width"</strong>.
                  </li>
                  <li>
                    Tap on it and enter the recommended DPI value shown above.
                  </li>
                </ol>
                <p className="mt-6 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl text-yellow-300 text-base">
                  <strong>Warning:</strong> Be careful! Entering a very high DPI
                  value can make your screen unreadable. Only use the
                  recommended value.
                </p>
              </InfoCard>
              ...
              <AIChatBox
                settings={{
                  ...generatedSettings.fixedSensitivity,
              ...
              <div className="text-center mt-12">
                <button
                  onClick={resetApp}
                  className="text-cyan-400 hover:text-white text-lg font-semibold transition-colors duration-300"
                >
        ...
        <footer className="w-full max-w-6xl mx-auto text-center text-gray-400 mt-20 py-8">
          ...
          <div className="space-y-2 text-sm">
            <p>
              &copy; {new Date().getFullYear()} Loading PC Free Sensi. All
              rights reserved.
            </p>
